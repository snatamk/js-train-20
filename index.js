...
Car.prototype.accelerate = function (speedMore) {
  this.speed += speedMore;
  console.log(
...
Car.prototype.brake = function (speedLess) {
  this.speed -= speedLess;
  console.log(
...
// Додатковий метод specific для прототипу Trucks, примає число якщо воно більше towingCapacity виводить рядок в консоль:
// Навантаження занадто важке для буксирування, якщо ні то рядок Тягнення навантаження...
Truck.prototype.specific = function (n) {
  if (n > this.towingCapacity) {
...
function ElectricCar(brand, model, year, mileage, batteryCapacity) {
  ...
  if (!new.target) {
    console.log(`error "Конструктор має бути викликаний з 'new'"`);
  } else {
